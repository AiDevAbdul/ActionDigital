...
const Experience = () => {
  return (
    <section id="experience" className="py-20 sm:py-32 border-t border-node-green/10 dark:border-node-green/10">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative pl-10 md:pl-16"> 
          ...
          <div className="absolute left-6 w-1 bg-node-green/20 dark:bg-node-green/20 h-full"></div> 
          ...
          {experienceData.map((item, index) => (
            ...
            >
                ...
                <div className="flex-shrink-0 relative mr-6 -ml-10 md:-ml-12">
                    ...
                    <div className="relative z-10 p-2 rounded-full bg-node-dark dark:bg-node-dark border border-node-green dark:border-node-green shadow-green-glow">
                      <item.icon className="w-5 h-5 text-node-green dark:text-node-green" />
                    </div>
                </div>

                {/* Content Card (Full width next to the icon) */}
                <div className="flex-grow">
                  <div className="p-6 rounded-lg bg-[#252525] dark:bg-[#252525] border border-node-green/30 dark:border-node-green/30 hover:border-node-green transition-all duration-300">
                    
                    <span className="text-sm font-semibold uppercase text-node-text-muted dark:text-node-text-muted flex items-center">
                        <Calendar className="w-3 h-3 mr-2" />
                        {item.duration}
                    </span>

                    <h3 className="mt-1 text-xl font-bold text-node-text-light dark:text-node-text-light">
                      {item.title}
                    </h3>
                    <p className={`text-node-green dark:text-node-green font-medium mb-3 ${item.type === 'education' ? 'text-node-light-green dark:text-node-light-green' : ''}`}>
                      {item.company}
                    </p>
                    
                    {item.details.length > 0 && (
                      <ul className="list-disc pl-5 space-y-1 text-node-text-muted dark:text-node-text-muted text-sm">
